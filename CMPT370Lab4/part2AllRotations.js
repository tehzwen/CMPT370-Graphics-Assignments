...
function setupKeypresses(state) {
    document.addEventListener("keydown", (event) => {
        ...
        let childrenObjects = getObjectByName(state, object.children);
        ...
        switch (event.code) {
            case "KeyA":
                if (event.getModifierState("Shift")) {
                    ...
                    if (state.hasSelected) {
                        translateArm(state, object, vec3.fromValues(0.1, 0.0, 0.0));
                    } else {
                        vec3.add(state.camera.center, state.camera.center, vec3.fromValues(0.1, 0.0, 0.0));
            ...
            case "KeyW":
                if (event.getModifierState("Shift")) {
                    ...
                    if (state.hasSelected) {
                        translateArm(state, object, vec3.fromValues(0.0, 0.0, -0.1));
                    } else {
            ...
            case "KeyQ":
                if (event.getModifierState("Shift")) {
                    if (state.hasSelected) {
                        rotateArm(state, "z", object, -0.1);
                    }
                } else {
                    // TODO: Move camera along Y axis
                    if (state.hasSelected) {
                        translateArm(state, object, vec3.fromValues(0.0, -0.1, 0.0));
                    } else {
                        vec3.add(state.camera.center, state.camera.center, vec3.fromValues(0.0, -0.1, 0.0));
...
function scaleVertices(verticesList, scaleVector) {
    for (let i = 0; i < verticesList.length; i++) {
        for (let j = 0; j < verticesList[i].length; j++) {
            verticesList[i][j] *= scaleVector;
        }
    }
}

function rotateObjectYInPlace(object, val) {
    let negativeCentroid = vec3.create();
    vec3.negate(negativeCentroid, object.centroid);
    vec3.add(object.model.position, object.model.position, negativeCentroid);
    mat4.rotateY(object.model.rotation, object.model.rotation, val);
    vec3.rotateY(object.centroid, object.centroid, vec3.fromValues(0.0, 0.0, 0.0), -val);
    vec3.add(object.model.position, object.model.position, object.centroid);
}

function rotateObjectXInPlace(object, val) {
    let negativeCentroid = vec3.create();
    vec3.negate(negativeCentroid, object.centroid);
    vec3.add(object.model.position, object.model.position, negativeCentroid);
    mat4.rotateX(object.model.rotation, object.model.rotation, val);
    vec3.rotateX(object.centroid, object.centroid, vec3.fromValues(0.0, 0.0, 0.0), -val);
    vec3.add(object.model.position, object.model.position, object.centroid);
}
...
function rotateArm(state, axis, arm, val) {
    ...
    if (axis === "x") {
        ...
        mat4.rotateX(arm.model.rotation, arm.model.rotation, val);
        vec3.rotateX(arm.centroid, arm.centroid, vec3.fromValues(0.0, 0.0, 0.0), -val);


        for (let i = 0; i < childrenObjects.length; i++) {
            vec3.add(childrenObjects[i].model.position, childrenObjects[i].model.position, negativeCentroid);
            mat4.rotateX(childrenObjects[i].model.rotation, childrenObjects[i].model.rotation, val);
            vec3.rotateX(childrenObjects[i].centroid, childrenObjects[i].centroid, vec3.fromValues(0.0, 0.0, 0.0), -val);
        ...
        //rotateObjectX(arm, val);
        mat4.rotateY(arm.model.rotation, arm.model.rotation, val);
        vec3.rotateY(arm.centroid, arm.centroid, vec3.fromValues(0.0, 0.0, 0.0), -val);


        for (let i = 0; i < childrenObjects.length; i++) {
            vec3.add(childrenObjects[i].model.position, childrenObjects[i].model.position, negativeCentroid);
            mat4.rotateY(childrenObjects[i].model.rotation, childrenObjects[i].model.rotation, val);
            vec3.rotateY(childrenObjects[i].centroid, childrenObjects[i].centroid, vec3.fromValues(0.0, 0.0, 0.0), -val);
            vec3.add(childrenObjects[i].model.position, childrenObjects[i].model.position, arm.centroid);
        }
        ...
        console.error("Incorrect axis, please use x y or z");
    }
}
...
function translateArm(state, arm, translation) {
    vec3.add(arm.model.position, arm.model.position, translation);
    vec3.add(arm.centroid, arm.centroid, translation);

    let childrenObjects = getObjectByName(state, arm.children);

    for (let i = 0; i < childrenObjects.length; i++) {
        vec3.add(childrenObjects[i].model.position, childrenObjects[i].model.position, translation);
        vec3.add(childrenObjects[i].centroid, childrenObjects[i].centroid, translation);
    }
}