    ...
    fileUploadButton.addEventListener("click", () => {
        ...
        fetch(url, {
            ...
            for (let i = 0; i < inputTriangles.length; i++) {
                ...
                //append 1.0 to the colors for alpha since data is only rgb
                let tempColorArray = inputTriangles[i].material.diffuse;
                tempColorArray.push(1.0);

                //use flat method to flatten the array of vertices
                inputVertices = inputVertices.concat(inputTriangles[i].vertices.flat());

                //create colors for every vertex
                for (let v = 0; v < inputTriangles[i].vertices.length; v++) {
                    inputColours = inputColours.concat(tempColorArray);
                }
            }
...
function initBuffers(gl, programInfo, inputVertices, inputIndices, inputColours) {
    ...
    const colours = new Float32Array(inputColours);
